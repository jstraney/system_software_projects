...
int execute_instruction (Instruction instruction) {
  ...
  // return from sub-procedure
    ...
    // clean up stack
    for (int i = tmp_sp; i > stack_pointer; i--) {
  ...
  // call sub-procedure
    ...
    for (int i = 0; i < VM_MAX_LEXI_LEVEL; i++) {
      ...
      if (lex_levels[i] == -1) {
        ...
        // mark the start of activation record
        lex_levels[i] = base_pointer;

        break;

      }
      else if ( i == VM_MAX_LEXI_LEVEL) {
...
int vm_print_trace (int status) {
  ...
  while (status == VM_STATUS_OK) {
    ...
    int *pipes = (int *)malloc(stack_pointer + 1 * (sizeof(int)));

    memset(pipes, 0, (stack_pointer + 1) * sizeof(int));
